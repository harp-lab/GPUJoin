...
void cpu_get_sort_merge_join_data(int *result, long long data_max_length,
                       int *relation_1_keys, int *relation_1_values, int relation_1_rows, int relation_1_columns, int relation_1_index,
                       int *relation_2_keys, int *relation_2_values, int relation_2_rows, int relation_2_columns, int relation_2_index) {
    long long row_count = 0, column_count = 0;
    int total_columns = relation_1_columns + relation_2_columns - 1;
    int relation_1_index_value, relation_2_index_value;
    long long r = 0, s = 0;
    ...
    while (r < relation_1_rows) {
        relation_1_index_value = relation_1_keys[r];
        relation_2_index_value = relation_2_keys[s];
        if (mark == -1) {
...
void sort_merge_join_relations(const char *data_path, char separator, const char *output_path,
                        int relation_1_rows, int relation_1_columns,
                        int relation_2_rows, int relation_2_columns,
                        int total_rows, int visible_rows) {
    std::chrono::high_resolution_clock::time_point time_point_begin;
    std::chrono::high_resolution_clock::time_point time_point_end;
    cout << "Sort-merge join operation" << endl;
    cout << "===================================" << endl;
    cout << "Relation 1: rows: " << relation_1_rows << ", columns: " << relation_1_columns << endl;
    cout << "Relation 2: rows: " << relation_2_rows << ", columns: " << relation_2_columns << "\n" << endl;
    ...
    int *relation_1_values = (int *) malloc(relation_1_rows * sizeof(int));
    int *relation_2_keys = (int *) malloc(relation_2_rows * sizeof(int));
    int *relation_2_values = (int *) malloc(relation_2_rows * sizeof(int));
    for (int i = 0; i < relation_1_rows; i++) {
    ...
    time_point_begin = chrono::high_resolution_clock::now();
    thrust::sort_by_key(thrust::host, relation_1_keys, relation_1_keys + relation_1_rows, relation_1_values);
    thrust::sort_by_key(thrust::host, relation_2_keys, relation_2_keys + relation_2_rows, relation_2_values);
    time_point_end = chrono::high_resolution_clock::now();
...
int main() {
    ...
    output_path = "output/join_cpu.txt";
